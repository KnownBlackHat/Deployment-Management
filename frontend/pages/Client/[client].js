...
export default function Client ({dbdata}) {
    ...
    if (dbdata){
        return(<>
            ...
            <main>
            <div className={`p-1 my-2 text-center text-white 
                bg-${dbdata.status==="online"?"green":"red"}-500 rounded`}>
            Server Name: {dbdata.name}</div>
            
            <div  className="h-[60vh] p-1 rounded text-sm bg-gray-700 
            overflow-y-scroll text-white">

            <span>Shell: </span>
            </div>
            <div id="status" className="flex flex-col items-center 
            justify-center mt-2 text-center md:space-x-4 
